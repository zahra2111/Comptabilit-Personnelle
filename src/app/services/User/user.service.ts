...
export class UserService {
  ...
  getModels(userId: number): Observable<Model[]> {
    return this.http.get<{ models: string[] }>(`${this.apiUrl}/${userId}`).pipe(
      switchMap(response => {
        const modelRequests = response.models.map(url => {
          const modelUrl = url.startsWith('/api') ? `${this.bankApiUrl}${url}` : `${this.bankApiUrl}/${url}`;
          console.log(`Fetching budget from: ${modelUrl}`);
          return this.http.get<Model>(modelUrl);
        });
